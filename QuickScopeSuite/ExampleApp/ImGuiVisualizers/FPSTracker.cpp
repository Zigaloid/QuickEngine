	...
	if (ImGui::Begin(windowTitle, pOpen, flags))
	{
		...
		if (ImGui::BeginMenuBar())
		{
			if (ImGui::BeginMenu("Display"))
			{
				...
				ImGui::Separator();
				ImGui::Checkbox("Show Statistics", &visConfig.showStatistics);

				ImGui::Separator();
				ImGui::Text("Graph Type:");
				const char* graphTypeNames[] = { "Bar Graph", "Line Graph", "Both" };
				int currentType = static_cast<int>(m_GraphType);
				if (ImGui::Combo("##GraphType", &currentType, graphTypeNames, IM_ARRAYSIZE(graphTypeNames)))
				{
					m_GraphType = static_cast<GraphType>(currentType);
				}
