...
void ProfilerController::HandleProfilerBinaryPacket(const std::vector<uint8_t>& messageBody)
{
    DECLARE_FUNC_VLOW();
...
void ProfilerController::HandleProfilerPacket(const std::string& messageBody)
{
    DECLARE_FUNC_VLOW();
...
bool ProfilerController::SaveSession(const std::string& filePath) 
{
    ...
    {
        AppDebug.printf("No profiler session data to save\n");
        return false;
    }
    ...
    if (result.HasError())
    {
        AppDebug.printf("Failed to save profiler session: %s\n", result.GetError().c_str());
        return false;
    }
    ...
    AppDebug.printf("Profiler session saved to %s (%u packets)\n", filePath.c_str(), packetCount);
    return true;
}

bool ProfilerController::LoadSession(const std::string& filePath)
{
    DECLARE_FUNC_LOW();
    ...
    {
        AppDebug.printf("Failed to load profiler session: %s\n", readResult.GetError().c_str());
        return false;
    }
    ...
    {
        AppDebug.printf("Invalid profiler session file (too small)\n");
        return false;
    }
    ...
    for (uint32_t i = 0; i < packetCount; ++i)
    {
        ...
        {
            AppDebug.printf("Invalid profiler session file (truncated at packet %u)\n", i);
            return false;
        }
        ...
        {
            AppDebug.printf("Invalid profiler session file (truncated data at packet %u)\n", i);
            return false;
        }
    ...
    AppDebug.printf("Profiler session loaded from %s (%u packets)\n", filePath.c_str(), packetCount);
    return true;
}

void ProfilerController::SaveSessionAs()
{
...
void ProfilerController::SaveSessionCurrent()
{
...
void ProfilerController::LoadSessionDialog()
{
...
void ProfilerController::ResetView()
{
    DECLARE_FUNC_VLOW();
...
void ProfilerController::registerFileActions()
{
    // File.Load
    m_actionManager.RegisterAction({
        .path = "File.Load",
        .description = "Load a profiler session from a file",
        .shortcutHint = "Ctrl+O",
...
void ProfilerController::registerProfilerActions()
{
    // Profiler.Start
    m_actionManager.RegisterAction({
        .path = "Profiler.Start",
    ...
    m_actionManager.RegisterAction({
        ...
        .targets = UI::ActionTarget::Menu | UI::ActionTarget::Console | UI::ActionTarget::Toolbar,
        .callback = [this]() { Clear(); m_sessionPackets.clear(); m_currentSessionPath.clear(); },
        .sortPriority = 30
    });
...
void ProfilerController::RenderSessionContent()
{
