...
/** @brief Mouse axis enumeration for movement and scroll. */
...
/** @brief Snapshot of all mouse input state for a single frame. */
struct MouseState
{
    ...
    bool  IsButtonPressed(MouseButton button) const { return buttons[static_cast<size_t>(button)]; }
    float GetAxis(MouseAxis axis)             const { return axes[static_cast<size_t>(axis)]; }
...
/** @brief Abstract mouse interface. */
class IMouse
{
public:
...
/** @brief Configuration parameters for mouse input. */
struct MouseConfig
{
