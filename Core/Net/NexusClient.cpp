...
bool CNexusClient::SendPipeMessage(const std::string& pipeName, const std::string& messageType, const std::string& body)
{
    if (!IsConnected()) return false;

    std::lock_guard<std::mutex> lock(m_sendMutex);
    std::string packet = BuildPipeMessagePacket(m_appName, pipeName, messageType, body);
    return SendAll(&m_socket, packet.data(), packet.size());
}
