...
namespace FileSystem {
    ...
    public:
        ...
        Result<size_t> Read(void* buffer, size_t bytesToRead) override {
            ...
            stream_.read(static_cast<char*>(buffer), bytesToRead);
        ...
        Result<size_t> Write(const void* buffer, size_t bytesToWrite) override {
            ...
            stream_.write(static_cast<const char*>(buffer), bytesToWrite);
        ...
        Result<uint64_t> Seek(int64_t offset, SeekOrigin origin) override {
            ...
            uint64_t pos = static_cast<uint64_t>(stream_.tellg());
        ...
        Result<FileInfo> GetInfo() override {
            ...
            FileInfo info = CreateFileInfo(path_);
        ...
        bool CanRead() const override {
            return isOpen_ && (mode_ == FileMode::Read || mode_ == FileMode::ReadWrite);
        }

        bool CanWrite() const override {
            return isOpen_ && (mode_ == FileMode::Write || mode_ == FileMode::ReadWrite || mode_ == FileMode::Append);
        }
        ...
        Result<void> WriteAllText(const std::string& text) override {
            ...
            stream_.write(text.c_str(), text.length());
    ...
    public:
        StandardDirectory(const std::string& path) : path_(path) {}
    ...
    public:
        ...
        Result<std::string> GetUserDirectory() override {
            // This is platform-specific, using environment variables as fallback
            std::string path;
        #if defined(_WIN32)
            char* buffer = nullptr;
            size_t len = 0;
