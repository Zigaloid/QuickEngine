...
namespace Reflection {
    ...
    struct ErrorInfo {
        ...
        // Add default constructor to avoid compilation issues
        ErrorInfo() : severity(ErrorSeverity::Error), category(ErrorCategory::Unknown), 
    ...
    // Custom exception class for reflection errors
    ...
    private:
        ErrorInfo m_error;
        mutable std::string m_whatString; // mutable for lazy evaluation in what()
    };

    // Error handler interface for different error handling strategies
    class IErrorHandler {
    public:
    ...
    public:
        DefaultErrorHandler(bool throwOnError = true, bool throwOnWarning = false)
    ...
    // Result class for operations that can fail - redesigned to avoid default constructor issues
    template<typename T>
    class Result {
    public:
