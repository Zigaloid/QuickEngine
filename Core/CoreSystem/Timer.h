...
public:
    ...
    {
        if (m_timeout.count() <= 0.0) return 1.0;
...
private:
    ...
    bool        m_paused  = false;  ///< Whether the timer is currently paused
    bool        m_looping = false;  ///< Whether the timer auto-resets on expiration
};
