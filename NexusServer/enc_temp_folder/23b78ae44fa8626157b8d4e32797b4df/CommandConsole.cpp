...
void CommandConsole::RefreshFunctionCallManagerCommands()
{
    ...
    // Also add console variables from the Console registry
    {
        auto varNames = Console::GetAllVarNames();
        for (const auto& vn : varNames) {
            auto v = Console::FindVar(vn);
            std::string desc = v ? v->description : "";
            RegisterCommand(vn, desc);
        }
    }
...
void CommandConsole::ExecuteCommand(const std::string& commandLine)
{
    ...
    // Check for console variables (get/set)
    {
        std::string trimmed = Trim(commandLine);
        // get first token (name)
        size_t spacePos = trimmed.find(' ');
        std::string varName = (spacePos == std::string::npos) ? trimmed : trimmed.substr(0, spacePos);
        auto var = Console::FindVar(varName);
        if (var) {
            if (spacePos == std::string::npos) {
                // get
                AddInfo(varName + " = " + var->GetAsString());
                return;
            } else {
                // set
                std::string value = Trim(trimmed.substr(spacePos + 1));
                if (var->SetFromString(value)) {
                    AddSuccess(varName + " set to " + var->GetAsString());
                } else {
                    AddError(std::string("Failed to set ") + varName + " to '" + value + "'");
                }
                return;
            }
        }
