...
namespace ImGuiVisualizers {
	...
	void PropertyWidgetMapEditor::PopulateMemberList(const std::string& className)
	{
		...
		// Collect hierarchy reflection maps (derived -> parent -> ...)
		...
		// Aggregate all members from the hierarchy, avoiding duplicates.
		// This lets the editor show inherited members alongside class-owned members.
		std::unordered_set<std::string> seen;
		for (const auto& pair : hierarchy) {
			if (pair.second == nullptr) continue;
			for (const auto& entry : *pair.second) {
				if (entry.GetProperty()) {
					const std::string name = entry.GetProperty()->GetName();
	...
	void PropertyWidgetMapEditor::LoadSelectedMemberMapping()
	{
		...
		// No local mapping — try to find an inherited mapping in the hierarchy.
		EditorWidgetType foundType = EditorWidgetType::Default;
		WidgetConfig foundCfg;
		std::string origin;
		if (PropertyWidgetMap::FindWidgetInHierarchy(cls, member, foundType, &foundCfg, &origin)) {
			m_editWidgetType = foundType;
			const auto originMap = PropertyWidgetMapRegistry::Instance().Get(origin);
			// Only set config flag if the origin map actually had config for that member.
			if (originMap && originMap->HasCustomWidget(member) && originMap->GetConfig(member)) {
				m_editHasConfig = true;
				m_editConfig = foundCfg;
			}
			...
			// No mapping anywhere in the hierarchy
			m_editWidgetType = EditorWidgetType::Default;
			m_editHasConfig = false;
			m_editConfig = WidgetConfig();
			m_editDisplayName.clear();
		...
		if (m_selectedClassIndex >= 0) {
			...
			if (m_memberNames.empty()) {
				...
				if (m_selectedMemberIndex < 0) {
					...
					// Show inherited hint when mapping comes from a parent class.
					if (m_mappingIsInherited && !m_mappingOriginClass.empty()) {
						ImGui::SameLine();
						ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.0f, 1.0f), "(Inherited from: %s)", m_mappingOriginClass.c_str());
					}
					ImGui::Separator();
