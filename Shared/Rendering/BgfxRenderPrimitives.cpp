...
namespace Rendering {
    ...
    void BgfxRenderPrimitives::RenderWireCone(bgfx::ViewId viewId, const float* worldMtx, uint32_t color)
    {
        if (!m_initialized)
            return;

        // Base circle (kWireSegments segments) + kWireSegments lateral edges from apex → base rim
        constexpr uint32_t numVerts = kWireSegments * 2 + kWireSegments * 2;
        PosColorVertex verts[numVerts];
        uint32_t idx = 0;

        // Unit cone: apex at (0, +0.5, 0), base circle at y = -0.5 with radius 0.5
        const float apex[3] = { 0.0f, 0.5f, 0.0f };

        for (int i = 0; i < kWireSegments; ++i)
        {
            const float t0 = bx::kPi2 * float(i) / float(kWireSegments);
            const float t1 = bx::kPi2 * float(i + 1) / float(kWireSegments);

            const float rim0[3] = { 0.5f * bx::cos(t0), -0.5f, 0.5f * bx::sin(t0) };
            const float rim1[3] = { 0.5f * bx::cos(t1), -0.5f, 0.5f * bx::sin(t1) };

            float wRim0[3], wRim1[3], wApex[3], wRim0b[3];
            TransformPoint(wRim0, rim0, worldMtx);
            TransformPoint(wRim1, rim1, worldMtx);
            TransformPoint(wApex, apex, worldMtx);
            TransformPoint(wRim0b, rim0, worldMtx);

            // Base circle edge
            verts[idx++] = { wRim0[0], wRim0[1], wRim0[2], color };
            verts[idx++] = { wRim1[0], wRim1[1], wRim1[2], color };

            // Lateral edge from apex to rim
            verts[idx++] = { wApex[0],  wApex[1],  wApex[2],  color };
            verts[idx++] = { wRim0b[0], wRim0b[1], wRim0b[2], color };
        }
