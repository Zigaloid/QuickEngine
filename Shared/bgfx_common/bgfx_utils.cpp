...
// --- add implementation near other loadShader / loadProgram implementations ---

bgfx::ShaderHandle loadShaderFromMemory(const void* _data, uint32_t _size, const bx::StringView& _name)
{
	if (NULL == _data || 0 == _size)
	{
		DBG("loadShaderFromMemory: invalid input data.");
		return BGFX_INVALID_HANDLE;
	}

	// Allocate a bgfx-managed block and copy the provided data.
	const bgfx::Memory* mem = bgfx::alloc(_size + 1);
	bx::memCopy(mem->data, _data, _size);
	// Ensure a trailing zero for any string-based consumers.
	((uint8_t*)mem->data)[_size] = '\0';

	bgfx::ShaderHandle handle = bgfx::createShader(mem);
	if (bgfx::isValid(handle) && !_name.isEmpty())
	{
		bgfx::setName(handle, _name.getPtr(), _name.getLength());
	}
...
/// Load a texture from an in-memory blob.
///
/// This helper assumes the blob is a texture container format that bgfx can parse
/// (for example KTX). It copies the data into bgfx-managed memory and calls
/// bgfx::createTexture on that memory.
///
/// Parameters:
///  - _data: pointer to raw file data (e.g. contents of a .ktx/.ktx2 etc.)
///  - _size: size of the data in bytes
///  - _name: optional name (only present for parity with other helpers; not used)
bgfx::TextureHandle loadTextureFromMemory(const void* _data, uint32_t _size, const char* _name)
{
	...
	// Copy data into bgfx-managed memory (bgfx takes ownership of this copy).
	const bgfx::Memory* mem = bgfx::copy(_data, _size);
	if (nullptr == mem)
	{
		return BGFX_INVALID_HANDLE;
	}

	// Let bgfx create the texture from the provided memory blob.
	// BGFX will parse supported container formats (KTX/PKM/etc.) and create the appropriate texture.
	// Use 0 for flags / skip here; callers can adapt flags if needed.
	bgfx::TextureHandle handle = bgfx::createTexture(mem, BGFX_TEXTURE_NONE, 0);
